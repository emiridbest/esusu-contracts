...
contract MiniSafeTokenStorageUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, IMiniSafeCommon {
    ...
    function initialize(address _initialOwner) external initializer {
        ...
        // M-5 Fix: Initialize base token info (cUSD)
        tokenInfo[cusdTokenAddress] = TokenInfo({
            isSupported: true,
            aTokenAddress: address(0xBba98352628B0B0c4b40583F593fFCb630935a45),
            totalShares: 0
        });

        // Initialize USDC
        address usdcAddress = address(0xcebA9300f2b948710d2653dD7B07f33A8B32118C);
        tokenInfo[usdcAddress] = TokenInfo({
            isSupported: true,
            aTokenAddress: address(0xFF8309b9e99bfd2D4021bc71a362aBD93dBd4785),
            totalShares: 0
        });
        supportedTokens.push(usdcAddress);

        // Initialize USDT
        address usdtAddress = address(0x48065fbBE25f71C9282ddf5e1cD6D6A887483D5e);
        tokenInfo[usdtAddress] = TokenInfo({
            isSupported: true,
            aTokenAddress: address(0xDeE98402A302e4D707fB9bf2bac66fAEEc31e8Df),
            totalShares: 0
        });
        supportedTokens.push(usdtAddress);
    }
