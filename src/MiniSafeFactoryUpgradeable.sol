...
contract MiniSafeFactoryUpgradeable is Ownable {
    ...
    function deployUpgradeableMiniSafe(
        ...
        // Deploy remaining contracts (these involve external calls)
        // Initialize TokenStorage and Integration with 'this' as owner to setup permissions
        addresses.tokenStorage = _deployTokenStorage(address(this));
        addresses.aaveIntegration = _deployAaveIntegration(addresses.tokenStorage, address(this), config.aaveProvider);
        
        // MiniSafe can be initialized with Timelock as owner immediately
        ...
        // Authorize MiniSafe to update user balances
        tokenStorage.setManagerAuthorization(addresses.miniSafe, true);
        // Authorize Integration to add supported tokens
        tokenStorage.setManagerAuthorization(addresses.aaveIntegration, true);

        // Transfer ownership to Timelock
        tokenStorage.transferOwnership(addresses.timelock);
        MiniSafeAaveIntegrationUpgradeable(addresses.aaveIntegration).transferOwnership(addresses.timelock);
