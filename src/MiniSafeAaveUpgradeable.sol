    ...
    IMiniSafeCommon 
{
    ...
    struct ThriftGroup {
        ...
        mapping(address => uint256) memberCollateral; // Collateral locked per member for this group
        bool isActive;
    ...
    // COLLATERAL STORAGE (for "Savings as Collateral" feature)
    /// @dev User's locked shares per token (cannot be withdrawn while locked)
    mapping(address => mapping(address => uint256)) public userLockedShares;
    
    /// @dev Tokens allowed for thrift groups (whitelist)
    ...
    event CollateralSeized(uint256 indexed groupId, address indexed member, uint256 amount);
    ...
    function initialize(
        ...
        // Allow tokens for thrift groups (whitelist)
        allowedThriftTokens[0x765DE816845861e75A25fCA122bb6898B8B1282a] = true; // cUSD
        allowedThriftTokens[address(0xcebA9300f2b948710d2653dD7B07f33A8B32118C)] = true; // USDC
        allowedThriftTokens[address(0x48065fbBE25f71C9282ddf5e1cD6D6A887483D5e)] = true; // USDT
    ...
    /**
     * @dev Get minimum amount scaled by token decimals
     * @param token Address of the token
     * @param baseAmount Base amount (assuming 18 decimals)
     * @return Scaled minimum amount
     */
    function _getMinAmount(address token, uint256 baseAmount) internal view returns (uint256) {
        uint8 decimals = IERC20Metadata(token).decimals();
        if (decimals == 18) return baseAmount;
        // Scale down/up from 18 decimals standard
        if (decimals < 18) {
            return baseAmount / (10 ** (18 - decimals));
        } else {
            return baseAmount * (10 ** (decimals - 18));
        }
    }
    ...
    function deposit(address tokenAddress, uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 minAmount = _getMinAmount(tokenAddress, MIN_DEPOSIT);
        require(amount >= minAmount, "Deposit amount too small");
    ...
    function createThriftGroup(
        ...
        require(tokenStorage.isValidToken(tokenAddress), "Unsupported token");
        uint256 minContrib = _getMinAmount(tokenAddress, MIN_CONTRIBUTION);
        require(contributionAmount >= minContrib, "Contribution amount too small");
        require(allowedThriftTokens[tokenAddress], "Token not allowed for thrift");
        ...
        // Lock collateral for public groups (5x contribution)
        if (isPublic) {
            _lockCollateralOnJoin(groupId, msg.sender, tokenAddress, contributionAmount);
        }
    ...
    function joinPublicGroup(uint256 groupId) external nonReentrant {
        ...
        // Strict Fairness: Cannot join once the group is active (cycles started)
        require(!group.isActive, "Group already active");

        // Lock collateral for public groups
        _lockCollateralOnJoin(groupId, msg.sender, group.tokenAddress, group.contributionAmount);

        group.members.push(msg.sender);

        // If group is now full, activate it and set up payout order
        if (group.members.length == group.maxMembers) {
            // Always set start date to activation time
            group.startDate = block.timestamp;
    ...
    function activateThriftGroup(uint256 groupId) external onlyGroupAdmin(groupId) {
        ...
        // Always set start date to activation time to prevent alignment issues
        group.startDate = block.timestamp;
        group.nextPayoutDate = block.timestamp + group.cycleDuration;
    ...
    function makeContribution(
        ...
        // Check sharesToMint
        // Check sharesToMint
        // import "forge-std/console.sol" needed?
        // Using event instead to avoid import mess in contract
        // emit DebugShares(assetsDeposited, sharesToMint, totalAssets, totalShares);
        uint256 sharesToMint;
        if (totalShares == 0 || totalAssets == 0) {
            sharesToMint = assetsDeposited;
        } else {
        ...
        // Unlock 1x collateral for public groups
        if (group.isPublic) {
            _unlockContributionCollateral(groupId, tokenAddress, amount);
        }
    ...
    function leaveGroup(uint256 groupId) external onlyGroupMember(groupId) nonReentrant {
        ...
        // Unlock collateral for public groups
        if (group.isPublic) {
            uint256 requiredCollateral = group.contributionAmount * MAX_MEMBERS;
            uint256 sharesToReclaim = convertToShares(tokenAddress, requiredCollateral);
            uint256 currentLocked = userLockedShares[msg.sender][tokenAddress];
            
            if (currentLocked > 0) {
                // If member has received payout, SEIZE their collateral (burn shares to cover debt)
                if (_hasReceivedPayout(groupId, msg.sender)) {
                    // 1. Release the lock first
                    _unlockCollateral(msg.sender, tokenAddress, groupId, currentLocked);
                    
                    // 2. Seize/Burn the shares
                    // Using false for isDeposit burns the shares from user
                    updateUserBalance(msg.sender, tokenAddress, currentLocked, false);
                    
                    emit CollateralSeized(groupId, msg.sender, currentLocked);
                } else {
                    // Normal release if no payout received
                    uint256 unlockAmount = currentLocked < sharesToReclaim ? currentLocked : sharesToReclaim;
                    _unlockCollateral(msg.sender, tokenAddress, groupId, unlockAmount);
                }
            }
        }
    ...
    /**
     * @dev Set allowed thrift token (owner only)
     */
    ...
    /**
     * @dev Get user's disposable (unlocked) balance
     */
    function getDisposableBalance(address user, address token) public view returns (uint256) {
        uint256 total = tokenStorage.getUserTokenShare(user, token);
        uint256 locked = userLockedShares[user][token];
        return total > locked ? total - locked : 0;
    }

    /**
     * @dev Convert assets to shares for a token
     */
    function convertToShares(address token, uint256 assets) public view returns (uint256) {
        uint256 totalAssets = aaveIntegration.getATokenBalance(token);
    ...
    /**
     * @dev Lock collateral for a user (internal)
     */
    function _lockCollateral(address user, address token, uint256 groupId, uint256 shares) internal {
        userLockedShares[user][token] += shares;
        thriftGroups[groupId].memberCollateral[user] = shares;
    }

    /**
     * @dev Unlock collateral for a user (internal)
     */
    function _unlockCollateral(address user, address token, uint256 groupId, uint256 shares) internal {
        if (userLockedShares[user][token] >= shares) {
            userLockedShares[user][token] -= shares;
        } else {
            userLockedShares[user][token] = 0;
        }
        if (thriftGroups[groupId].memberCollateral[user] >= shares) {
            thriftGroups[groupId].memberCollateral[user] -= shares;
        } else {
            thriftGroups[groupId].memberCollateral[user] = 0;
        }
    }

    /**
     * @dev Lock 5x collateral when joining a public group
     */
    function _lockCollateralOnJoin(uint256 groupId, address user, address token, uint256 contribution) internal {
        uint256 requiredCollateral = contribution * MAX_MEMBERS;
        uint256 sharesNeeded = convertToShares(token, requiredCollateral);
        require(getDisposableBalance(user, token) >= sharesNeeded, "Insufficient collateral");
        _lockCollateral(user, token, groupId, sharesNeeded);
    }

    /**
     * @dev Cover a defaulter's contribution using their collateral
     */
        ...
        nonReentrant 
    {
        ...
        _seizeCollateralForContribution(groupId, defaulter);
    }

    /**
     * @dev Seize collateral to cover a contribution
     */
    function _seizeCollateralForContribution(uint256 groupId, address defaulter) internal {
        ThriftGroup storage group = thriftGroups[groupId];
        uint256 amount = group.contributionAmount;
        uint256 sharesNeeded = convertToShares(group.tokenAddress, amount);
        
        require(userLockedShares[defaulter][group.tokenAddress] >= sharesNeeded, "Insufficient collateral");
        
        // Unlock and transfer to pool
        _unlockCollateral(defaulter, group.tokenAddress, groupId, sharesNeeded);
        updateUserBalance(defaulter, group.tokenAddress, sharesNeeded, false);
        updateUserBalance(address(this), group.tokenAddress, sharesNeeded, true);
        
        // Update thrift accounting
        totalThriftStaked[group.tokenAddress] += amount;
    ...
    /**
     * @dev Unlock 1x collateral when contribution is made
     */
    function _unlockContributionCollateral(uint256 groupId, address tokenAddress, uint256 amount) internal {
        uint256 cycleShares = convertToShares(tokenAddress, amount);
        ThriftGroup storage group = thriftGroups[groupId];
        
        if (group.memberCollateral[msg.sender] >= cycleShares) {
            _unlockCollateral(msg.sender, tokenAddress, groupId, cycleShares);
        }
    }
